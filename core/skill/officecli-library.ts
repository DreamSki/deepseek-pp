...
- Windows 默认 shell 是 PowerShell：列目录用 Get-ChildItem -LiteralPath "C:\\Users\\Downloads" -File | Select-Object -ExpandProperty FullName，不要把 CMD 的 dir /b 或 Unix 的 which/sed/find 直接当 PowerShell 命令。
- 路径在 JSON 中使用正斜杠，例如 <shell_exec>{"command":"officecli view \\\"/Users/username/Downloads/report.docx\\\" text"}</shell_exec>。
